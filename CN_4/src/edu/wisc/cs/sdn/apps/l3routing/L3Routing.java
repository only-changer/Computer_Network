        ...
        ILinkDiscoveryListener, IDeviceListener
{
    ...
    public void maintain(){
        ...
        for (Host host : hosts)
        {
            ...
            Map<IOFSwitch, Integer> cost = new HashMap<IOFSwitch, Integer>();
            Map<IOFSwitch, Integer> port = new HashMap<IOFSwitch, Integer>();

            cost.put(host.getSwitch(), 1);
            port.put(host.getSwitch(), host.getPort());
            boolean changed = true;
            Integer check = 0;
            while (changed || check < 100)
            {
                ++check;
                changed = false;
                for (Link link : links)
                {
                    IOFSwitch src = switches.get(link.getSrc());
                    IOFSwitch dst = switches.get(link.getDst());

                    if (cost.containsKey(src))
                    {
                        if (cost.containsKey(dst))
                        {
                            if (cost.get(src) > cost.get(dst) + 1)
                            {
                                cost.put(src, cost.get(dst) + 1);
                                port.put(src, link.getSrcPort());
                                changed = true;
                            }
                        }
                    }
                    ...
                    if (cost.containsKey(dst))
                    {
                        if (cost.containsKey(src))
                        {
                            if (cost.get(dst) > cost.get(src) + 1)
                            {
                                cost.put(dst, cost.get(src) + 1);
                                port.put(dst, link.getDstPort());
                                changed = true;
                            }
                        }
                    }
            ...
            for (IOFSwitch sw : switches.values())
            {
                ...
                if (!port.containsKey(sw))
                    continue;
                OFAction act = new OFActionOutput(port.get(sw));
                OFInstruction ins = new OFInstructionApplyActions(Arrays.asList(act));
                SwitchCommands.installRule(sw, table, SwitchCommands.DEFAULT_PRIORITY, match, Arrays.asList(ins));
            }
        }
    }
